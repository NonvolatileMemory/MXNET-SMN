...
def ParseSingleTurn():
    ...
    pickle.dump([revs, word2vec, max_len,createtopicvec()], open("ubuntu_data.test",'wb'))
    logger.info("dataset created!")

def ParseMultiTurn():
    logging.basicConfig(format='%(asctime)s : %(levelname)s : %(message)s', level=logging.INFO)
    revs, vocab, max_len = build_multiturn_data(r"/root/桌面/DoubanConversaionCorpus/train.txt",isshuffle=False)
    word2vec = WordVecs(r"/root/桌面/DoubanConversaionCorpus/train_vec.model", vocab, True, True)
    pickle.dump([revs, word2vec, max_len], open("train_processed",'wb'))
    logger.info("dataset created!")
